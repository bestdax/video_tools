...
def trim_silence(
    ...
    duration = _get_duration(in_filename)
    ...
    for i, (start_time, end_time) in enumerate(chunk_times):
        ...
        out_filename = os.path.join(tempdir, temp_segments_pattern.format(i, i=i))
        concat_files.append(f'file {out_filename}')

        logger.info(
            '{}: start={:.02f}, end={:.02f}, duration={:.02f}, {:02d}%'
            .format(out_filename, start_time, end_time,
                    time,
                    math.floor(
                        end_time / duration * 100 if i != len(
                            chunk_times) - 1 else 100)
                    )
        )
        _logged_popen(
            (ffmpeg
    ...
    output_filename = in_filename_splits[0] + '_out' + in_filename_splits[1]
